...
Write-Host ""
Write-Host "Step 2: Building self-contained .exe..." -ForegroundColor Yellow
...
$cliAppImageDir = "dist-exe\NCSDecompCLI"
$cliAppImageExe = "dist-exe\NCSDecompCLI\NCSDecompCLI.exe"

if (Test-Path $cliAppImageDir) {
    Copy-Item $cliAppImageDir "$publishDir\NCSDecompCLI" -Recurse
    Write-Host "  - Copied NCSDecompCLI folder (portable app)" -ForegroundColor Cyan
    Write-Host "    Run: NCSDecompCLI\NCSDecompCLI.exe" -ForegroundColor Gray
} else {
...
$guiAppImageDir = "dist-exe\NCSDecomp"
$guiAppImageExe = "dist-exe\NCSDecomp\NCSDecomp.exe"

if (Test-Path $guiAppImageDir) {
    Copy-Item $guiAppImageDir "$publishDir\NCSDecomp" -Recurse
    Write-Host "  - Copied NCSDecomp folder (portable app)" -ForegroundColor Cyan
    Write-Host "    Run: NCSDecomp\NCSDecomp.exe" -ForegroundColor Gray
} else {
...
# Copy required nwscript files
if (Test-Path "k1_nwscript.nss") {
    Copy-Item "k1_nwscript.nss" "$publishDir\"
    Write-Host "  - Copied k1_nwscript.nss" -ForegroundColor Cyan
}
if (Test-Path "tsl_nwscript.nss") {
    Copy-Item "tsl_nwscript.nss" "$publishDir\"
    Write-Host "  - Copied tsl_nwscript.nss" -ForegroundColor Cyan
}

# Copy user-friendly README
if (Test-Path "README-USER.md") {
